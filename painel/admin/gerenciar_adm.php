        ...
        <div class="col-md-9 col-12">
            ...
            <div class="box"> 
                ...
                <?php 
                    ...
                    $query_total = "select count(*) from user where type = 2";
                    ...
                    if(($page > 0) && ($page <= $max_pages)){
                        ...
                        $query = "select * from user where type = 2 order by name, last_name limit " . $limit . " offset " . $offset;
                    ...
                    if($stmt->rowCount() > 0) {
                            ...
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    $can_delete = "";
                                    if($id_get != $_SESSION['user_id']){
                                        $can_delete = "<button class='btn btn-sm m-1 btn-error delete' id={$id_cript_to_del} email-data={$email} data-toggle='tooltip' data-placement='top' title='Deletar usuário'><i class='fas fa-trash'></i></button>";
                                    }

                                    if(!$status){
                                        $is_disable = "<button class='btn btn-sm m-1 btn-success reactivate' id={$id_cript_to_up} email-data={$email} data-toggle='tooltip' data-placement='top' title='Reativar usuário' {$is_disable}><i class='fas fa-undo'></i></button>";
                                    }else{
                                        $is_disable = "<button class='btn btn-sm m-1 btn-warn disable-btn' id={$id_cript_to_up} email-data={$email} data-toggle='tooltip' data-placement='top' title='Desativar usuário' {$is_disable}><i class='fas fa-times-circle'></i></button>";
                                    }

                                    $res .= "<tr id='student-" . $user ."'><td>{$imagem}</td><td class='text-center'> ".$name." ".$last_name." </td><td class='text-center'> ".$email." </td><td class='text-center'> ".$status." </td><td class='text-center'>{$can_delete}{$is_disable}<a href='{$configBase}/admin/editar_conta/".$user."' class='btn btn-sm btn-primary m-1' data-toggle='tooltip' data-placement='top' title='Editar usuário'><i class='fas fa-edit'></i></a></td></tr>";
